...
class JmsVerMegaMenu extends Module
{
    ...
    public function install()
    {
        if (parent::install() && $this->registerHook('header') && $this->registerHook('displayTopColumn') && $this->installSamples()) {
            $res = true;
            Configuration::updateValue('JMSVMM_MOUSEEVENT', 'hover');
            Configuration::updateValue('JMSVMM_DURATION', 200);
            Configuration::updateValue('JMSVMM_LOADBOOTSTRAPCSS', 0);
            $tab_parent_id = $this->getJmsModulesTab();
            $id_tab1 = $this->addTab('Jms Vertical MegaMenu', 'dashboard', $tab_parent_id, 4, 'menu');
