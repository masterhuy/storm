		...
		<div class="blog-item">
			...
			{if $show_time == '1'}
				<div class="time">
					...
					<span class="month">
						{$post.created|escape:'html':'UTF-8'|date_format:'%b'}
					</span>
				</div>
			{/if}
			{if $show_category == '1'}
			...
			<div class="post-info">
				...
				{/if}
				{if $show_readmore == '1'}	
					<a class="btn-readmore btn-active" href="{jmsblog::getPageLink('jmsblog-post', $params)|escape:'htmlall':'UTF-8'|replace:'&amp;':'&'}">{l s='Read more' d='Modules.JmsPagebuilder'}</a>	
				{/if}
			</div>
		</div>		
	{/foreach}	
